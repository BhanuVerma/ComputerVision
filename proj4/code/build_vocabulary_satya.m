...
for i = 1:N
   ...
   [locations, feats] = vl_dsift(single(I),'step',5);
   SIFT_features = [SIFT_features,feats];
end
samples = 1000;
N = size(SIFT_features, 2);
indices = randsample(N,samples);
sampled_SIFT_features = SIFT_features(:,indices);
[centres,assignments] = vl_kmeans(single(sampled_SIFT_features),vocab_size);
vocab = centres';
end
