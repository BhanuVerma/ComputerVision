...
X=X';
y1=ones(size(features_pos,1),1);
y2=-1*ones(size(features_neg,1),1);
Y=[y1;y2];
Y=Y';
...
% trianing data.
features_hard_neg=get_hard_neg(non_face_scn_path,w ,b, feature_params)
X=[X;features_hard_neg];
y3=-1*ones(size(features_hard_neg,1),1);
Y=[Y;y3];
X=X';
Y=Y';
[w1 b1]=vl_svmtrain(X,Y,0.0001);
